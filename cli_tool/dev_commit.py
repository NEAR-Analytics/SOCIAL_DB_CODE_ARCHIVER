...
widget_names_set = {row['widget_name'] for row in snowflake_data}  # Use set for O(1) lookups

for widget_name in widget_names_set:
    ...
    try:
        ...
        for near_dev, widget_entries in dev_widgets.items():
            ...
            for widget_entry in widget_entries:
                ...
                if current_source != source_code:
                    with open(source_code_file, 'w') as f:
                        f.write(source_code)
                    has_changes = True

                widget_details_file = os.path.join(widget_folder, COMMIT_RAW_FILENAME)
                if os.path.exists(widget_details_file):
                    with open(widget_details_file, 'r') as f:
                        current_details = json.load(f)
                else:
                    current_details = None

                if current_details != widget_entry:
